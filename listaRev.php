...
$q = "SELECT Cod,NumeRev, Coperta, Pret, Descriere From reviste";

$res = mysql_query($q);
while($row = mysql_fetch_array($res))
{
	echo '<tr><font color = "blue"><b>'. $row['NumeRev'].'</b></font><br><img width="200" height="275" src = "img/'.$row['Coperta'].'">'.'<br><font color = "red">Pret:'.$row['Pret'].'</font><br><font color = "black">'.$row['Descriere'].'</font><br><tr>';
	echo '<form action = "adaugaAbonamentList.php" method = "POST"><input type ="hidden" name = "COD" value = "'.$row['Cod'].'"><font color ="color"><input type ="submit" value = "ABONEAZATE"></form></form><br><br>';
}
?></td></table>
		</div>
        </td>
