...
$q = "SELECT NumeRev, Coperta, Pret From reviste, abonamente, abonat WHERE reviste.Cod = abonamente.Cod AND abonamente.CNP = abonat.CNP AND Nume LIKE '%".$_POST['NUME']."%'";
...
while($row = mysql_fetch_array($res))
{
	...
	$i++;
	$s = $s + $row['Pret'];
	echo '<td><font color = "blue"><b>'. $row['NumeRev'].'</b></font><br><img width="125" height=""25 src = "img/'.$row['Coperta'].'">'.'<br><font color = "red">Pret:'.$row['Pret'].'</font><br></td>';
	if($i % 3 == 0)
		echo'</tr>';
}

echo'</table>';
echo'<br><font size = "6" color = "black">Pretul lunar total:'.$s.'</font>';
?>
		</div>
        </td>
       </tr>
