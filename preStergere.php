...
$q = "SELECT Nume, NumeRev, CodRef,Data1, Data2 FROM abonat, abonamente, reviste WHERE abonat.CNP = abonamente.CNP AND reviste.Cod = abonamente.Cod AND Nume LIKE '%".$_POST['NUME']."%'";

$res = mysql_query($q);
while($r = mysql_fetch_array($res))
	{echo '<br><font color = "red" size = "3">Nume:'.$r['Nume'].'</font> <font color = "black" size = "3"><br>NumeRevista:'.$r['NumeRev'].' <br>Data1:'.$r['Data1'].' <br>Data2:'. $r['Data2'].'</font> <br>'." <a href = 'Stergere.php?n=".$r['CodRef']."'> Stergere </a>";
		$c = 1;
	}
	if($c == 0)
		echo'<font color = "black"><h3>Nu a fost gasit nici un abonament </h3></font>';
?>
		</div>
        </td>
       </tr>
